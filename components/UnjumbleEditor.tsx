...
interface Props {
  questions: UnjumbleQuestion[]
  onChange: (questions: UnjumbleQuestion[]) => void
}

export default function UnjumbleEditor({ questions, onChange }: Props) {
  const [uploadingIdx, setUploadingIdx] = useState<number | null>(null)
  const [uploadError, setUploadError] = useState('')

  const updateQuestion = (index: number, field: keyof UnjumbleQuestion, value: string | undefined) => {
  ...
  const addQuestion = () => {
    onChange([...questions, { id: questions.length + 1, word: '', clue: '', image_url: '' }])
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-[10px] text-gray-400 bg-[#e6f0fa] rounded-lg px-3 py-2">
        Single word → letters get scrambled. Sentence with spaces → words get scrambled.
      </p>
      ...
      {questions.map((q, i) => (
        <div key={i} className="bg-[#f7fafd] rounded-xl p-4 border border-[#e6f0fa]">
          ...
          {/* Clue */}
          <div className="mb-3">
            <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Clue (optional)</label>
            <input
              type="text"
              value={q.clue || ''}
              onChange={(e) => updateQuestion(i, 'clue', e.target.value)}
              placeholder="e.g. A place where children learn"
              className="w-full px-3 py-2 text-sm text-[#46464b] border border-[#cddcf0] rounded-lg focus:outline-none focus:border-[#416ebe] transition-colors"
            />
          </div>

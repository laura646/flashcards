...
// IELTS Reading runner — Flow-chart completion (Type 12).
//
// ADDITIVE: not imported by any live file. Self-contained student runner.
// A vertical sequence of connected step boxes (downward arrow connectors), some
// boxes containing gaps. Two variants (spec §12), both handled here:
...
//
// Checking (spec §12): 'passage' → checkAccepted per gap; 'word_bank' →
// checkLetter(assignedOptionId, gap.correctOptionId).

import { useMemo, useState } from 'react'
import type { FlowChartCompletionGroup, FlowChartSegment } from '@/lib/ielts/types'
...
export interface ReadingFlowChartCompletionRunnerProps {
...
const isGap = (s: FlowChartSegment): s is GapSegment => s.type === 'gap'

export function ReadingFlowChartCompletionRunner({
  group,
  onScore,
  className,
}: ReadingFlowChartCompletionRunnerProps) {
  const { steps, instruction, variant, wordBank, wordLimit: groupLimit } = group
  const isBank = variant === 'word_bank'

  /** Every gap across every step box, in order. */
  const gaps = useMemo<GapSegment[]>(
    () => steps.flatMap((step) => step.segments.filter(isGap)),
    [steps],
  )
  ...
  return (
    <RunnerShell
      ...
      className={className}
    >
      {/* Word bank (word-list variant only), shown above the flow chart. */}
      ...
      {/* Vertical step boxes joined by downward arrow connectors. */}
      <div className="flex flex-col items-stretch">
        {steps.map((step, si) => (
          <div key={`step-${si}`} className="flex flex-col items-center">
            <div className="w-full rounded-card border-[1.5px] border-sky-border bg-white px-4 py-3">
              <p className="flex flex-wrap items-baseline gap-x-1.5 gap-y-2 text-[14px] leading-relaxed text-ink-black">
                {step.segments.map((seg, gi) =>
                  seg.type === 'text' ? (
                    <span key={`t-${si}-${gi}`}>{seg.text}</span>
                  ) : isBank ? (
                    renderBankSlot(seg)
                  ) : (
                    <GapField
                      ...
                      checked={checked}
                      ariaLabel={`Flow-chart gap ${seg.number}`}
                    />
                  ),
                )}
              </p>
            </div>
            {/* Downward arrow connector between boxes (not after the last). */}
            {si < steps.length - 1 && (
              <div
                aria-hidden="true"
                className="flex flex-col items-center text-sky-text my-1.5"
              >
                <span className="block h-4 w-px bg-sky-border" />
                <span className="-mt-1 text-[14px] leading-none">▼</span>
              </div>
            )}
          </div>
        ))}
...
export default ReadingFlowChartCompletionRunner
